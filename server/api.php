...
if (isset($_GET['function_code']) && $_GET['function_code'] == 'getCustomerTbleData') {
    ...
    // Check if user session is still valid (customer)
    ob_clean();
    header('Content-Type: application/json');
    ...
    ob_end_flush();
    exit;
} else if (isset($_GET['function_code']) && $_GET['function_code'] == 'checkAdminSession') {
    // Check if admin/employee session is still valid
    ob_clean();
    header('Content-Type: application/json');
